...
class GyroscopeMeasurementsGeneratorTest implements GyroscopeMeasurementsGeneratorListener {
    ...
    private final List<BodyKinematicsSequence<StandardDeviationTimedBodyKinematics>> sequences = new ArrayList<>();

    private final List<StandardDeviationBodyKinematics> measurements = new ArrayList<>();

    private final AccelerometerMeasurementsGeneratorListener accelerometerGeneratorListener =
    ...
    void testConstructor1() {
        final var generator = new GyroscopeMeasurementsGenerator();
    ...
    void testConstructor2() {
        final var generator = new GyroscopeMeasurementsGenerator(this);
    ...
    void testProcessCalibrateAndResetWithNoiseMaCommonAxisAndNoGDependentCrossBiases() throws WrongSizeException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            final var accelerometerGenerator = new AccelerometerMeasurementsGenerator(accelerometerGeneratorListener);
            ...
            for (var i = 0; i < n; i++) {
                ...
                final var sequence = generateDynamicSamples(generator, accelerometerGenerator, dynamicPeriodLength, 
                        trueKinematics, randomizer, ecefFrame, nedFrame, errors, start, false);
                seqs.add(sequence);
                start += dynamicPeriodLength;

                if (dynamicIntervalDetected != i + 1) {
                    failed = true;
                    break;
                }
                assertEquals(dynamicIntervalDetected, i + 1);
                assertEquals(this.sequences.size(), i);
            }
            ...
            final var trueCalibrator = new EasyGyroscopeCalibrator(seqs, true,
                    false, initialBg, initialMg, initialGg, ba, ma);
            final var calibrator = new EasyGyroscopeCalibrator(this.sequences, true, 
                    false, initialBg, initialMg, initialGg, ba, ma);
    ...
    void testProcessCalibrateAndResetSmallNoiseMaGeneralAndWithGDependentCrossBiases() throws WrongSizeException,
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                assertEquals(i, this.sequences.size());
            }
    ...
    @Test
    void testProcessCalibrateAndResetSmallNoiseMaCommonAxisAndWithGDependentCrossBiases() throws WrongSizeException,
            InvalidSourceAndDestinationFrameTypeException, LockedException, NotReadyException,
            InvalidRotationMatrixException, RotationException {
    ...
    void testProcessSkipStaticInterval() throws WrongSizeException, InvalidSourceAndDestinationFrameTypeException,
        ...
        var start = initialStaticSamples;
        // generate static samples
        generateStaticSamples(generator, accelerometerGenerator, staticPeriodLength, trueKinematics, errors, start);
        start += staticPeriodLength;
        ...
        assertNotNull(generateDynamicSamples(generator, accelerometerGenerator, dynamicPeriodLength, trueKinematics,
                randomizer, ecefFrame, nedFrame, errors, start, false));
    ...
    void testProcessCalibrateAndResetSmallNoiseWithRotationAndPositionChangeMaCommonAxisAndNoGDependentCrossBiases()
        ...
        for (var t = 0; t < TIMES; t++) {
            ...
            for (var i = 0; i < n; i++) {
                ...
                // generate dynamic samples
                final var sequence = generateDynamicSamples(generator, accelerometerGenerator, dynamicPeriodLength,
                        trueKinematics, randomizer, ecefFrame, nedFrame, errors, start, true);
                seqs.add(sequence);
                start += dynamicPeriodLength;
    ...
    private static boolean generateStaticSamples(
            ...
            final BodyKinematics trueKinematics, final IMUErrors errors, final int startSample) throws LockedException {
        ...
        for (int i = 0, j = startSample; i < numSamples; i++, j++) {
            ...
            final var result1 = generator.process(timedMeasuredKinematics);
            final var result2 = accelerometerGenerator.process(measuredKinematics);
