...
public class NEDKinematicsEstimatorTest {
    ...
    public void testEstimateAndReturnNew() throws InvalidSourceAndDestinationFrameTypeException,
        ...
        final BodyKinematics k1 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC, vn, ve, vd,
                oldVn, oldVe, oldVd, latitude, height, oldLatitude, oldHeight);

        final Time timeInterval = new Time(TIME_INTERVAL_SECONDS, TimeUnit.SECOND);
        final BodyKinematics k2 = estimator.estimateAndReturnNew(timeInterval, c, oldC, vn, ve, vd,
                oldVn, oldVe, oldVd, latitude, height, oldLatitude, oldHeight);
        ...
        final BodyKinematics k4 = estimator.estimateAndReturnNew(timeInterval, c, oldC, velocity, oldVelocity,
                latitude, height, oldLatitude, oldHeight);
        ...
        final Distance heightDistance = new Distance(height, DistanceUnit.METER);
        final Distance oldHeightDistance = new Distance(oldHeight, DistanceUnit.METER);
        final BodyKinematics k5 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC, velocity, oldVelocity,
                latitudeAngle, heightDistance, oldLatitudeAngle, oldHeightDistance);

        final BodyKinematics k6 = estimator.estimateAndReturnNew(timeInterval, c, oldC, velocity, oldVelocity,
                latitudeAngle, heightDistance, oldLatitudeAngle, oldHeightDistance);

        final NEDPosition position = new NEDPosition(latitude, longitude, height);
        final NEDPosition oldPosition = new NEDPosition(oldLatitude, oldLongitude, oldHeight);
        final BodyKinematics k7 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC, vn, ve, vd,
                oldVn, oldVe, oldVd, position, oldPosition);

        final BodyKinematics k8 = estimator.estimateAndReturnNew(timeInterval, c, oldC, vn, ve, vd, oldVn, oldVe, oldVd,
        ...
        final Speed oldSpeedE = new Speed(oldVe, SpeedUnit.METERS_PER_SECOND);
        final Speed oldSpeedD = new Speed(oldVd, SpeedUnit.METERS_PER_SECOND);
        final BodyKinematics k9 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC, speedN, speedE, speedD,
                oldSpeedN, oldSpeedE, oldSpeedD, position, oldPosition);

        final BodyKinematics k10 = estimator.estimateAndReturnNew(timeInterval, c, oldC, speedN, speedE, speedD,
                oldSpeedN, oldSpeedE, oldSpeedD, position, oldPosition);

        final BodyKinematics k11 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC, velocity, oldVelocity,
                position, oldPosition);

        final BodyKinematics k12 = estimator.estimateAndReturnNew(timeInterval, c, oldC, velocity, oldVelocity,
                position, oldPosition);

        final BodyKinematics k13 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, frame, oldC,
                oldVn, oldVe, oldVd, oldLatitude, oldHeight);

        final BodyKinematics k14 = estimator.estimateAndReturnNew(timeInterval, frame, oldC, oldVn, oldVe, oldVd,
                oldLatitude, oldHeight);

        final BodyKinematics k15 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, frame, oldC, oldVelocity,
                oldLatitude, oldHeight);

        final BodyKinematics k16 = estimator.estimateAndReturnNew(timeInterval, frame, oldC, oldVelocity, oldLatitude,
                oldHeight);

        final BodyKinematics k17 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, frame, oldC, oldVelocity,
                oldLatitudeAngle, oldHeightDistance);

        final BodyKinematics k18 = estimator.estimateAndReturnNew(timeInterval, frame, oldC, oldVelocity,
                oldLatitudeAngle, oldHeightDistance);

        final BodyKinematics k19 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, frame, oldC, oldVelocity,
                oldLatitude, oldHeight);

        final BodyKinematics k20 = estimator.estimateAndReturnNew(timeInterval, frame, oldC, oldVelocity, oldLatitude,
                oldHeight);

        final BodyKinematics k21 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, frame, oldC,
                oldVn, oldVe, oldVd, oldPosition);

        final BodyKinematics k22 = estimator.estimateAndReturnNew(timeInterval, frame, oldC, oldVn, oldVe, oldVd,
                oldPosition);

        final BodyKinematics k23 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, frame, oldC, oldSpeedN,
                oldSpeedE, oldSpeedD, oldPosition);

        final BodyKinematics k24 = estimator.estimateAndReturnNew(timeInterval, frame, oldC, oldSpeedN, oldSpeedE,
                oldSpeedD, oldPosition);

        final BodyKinematics k25 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, frame, oldC, oldVelocity,
                oldPosition);

        final BodyKinematics k26 = estimator.estimateAndReturnNew(timeInterval, frame, oldC, oldVelocity, oldPosition);

        final BodyKinematics k27 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, vn, ve, vd, latitude,
                height, oldFrame);

        final BodyKinematics k28 = estimator.estimateAndReturnNew(timeInterval, c, vn, ve, vd, latitude, height,
                oldFrame);

        final BodyKinematics k29 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, velocity, latitude, height,
                oldFrame);

        final BodyKinematics k30 = estimator.estimateAndReturnNew(timeInterval, c, velocity, latitude, height,
                oldFrame);

        final BodyKinematics k31 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, velocity, latitudeAngle,
                heightDistance, oldFrame);

        final BodyKinematics k32 = estimator.estimateAndReturnNew(timeInterval, c, velocity, latitudeAngle,
                heightDistance, oldFrame);

        final BodyKinematics k33 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, vn, ve, vd, position,
                oldFrame);

        final BodyKinematics k34 = estimator.estimateAndReturnNew(timeInterval, c, vn, ve, vd, position, oldFrame);

        final BodyKinematics k35 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, speedN, speedE, speedD,
                position, oldFrame);

        final BodyKinematics k36 = estimator.estimateAndReturnNew(timeInterval, c, speedN, speedE, speedD, position,
                oldFrame);

        final BodyKinematics k37 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, velocity, position,
                oldFrame);

        final BodyKinematics k38 = estimator.estimateAndReturnNew(timeInterval, c, velocity, position, oldFrame);

        final BodyKinematics k39 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, frame, oldFrame);

        final BodyKinematics k40 = estimator.estimateAndReturnNew(timeInterval, frame, oldFrame);

        final BodyKinematics k41 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC,
                speedN, speedE, speedD, oldSpeedN, oldSpeedE, oldSpeedD, latitude, height, oldLatitude, oldHeight);

        final BodyKinematics k42 = estimator.estimateAndReturnNew(timeInterval, c, oldC, speedN, speedE, speedD,
                oldSpeedN, oldSpeedE, oldSpeedD, latitude, height, oldLatitude, oldHeight);

        final BodyKinematics k43 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC, vn, ve, vd,
                oldVn, oldVe, oldVd, latitudeAngle, height, oldLatitudeAngle, oldHeight);

        final BodyKinematics k44 = estimator.estimateAndReturnNew(timeInterval, c, oldC, vn, ve, vd,
                oldVn, oldVe, oldVd, latitudeAngle, height, oldLatitudeAngle, oldHeight);

        final BodyKinematics k45 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC, vn, ve, vd,
                oldVn, oldVe, oldVd, latitude, heightDistance, oldLatitude, oldHeightDistance);

        final BodyKinematics k46 = estimator.estimateAndReturnNew(timeInterval, c, oldC, vn, ve, vd,
                oldVn, oldVe, oldVd, latitude, heightDistance, oldLatitude, oldHeightDistance);

        final BodyKinematics k47 = estimator.estimateAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC,
                speedN, speedE, speedD, oldSpeedN, oldSpeedE, oldSpeedD, latitudeAngle, heightDistance,
                oldLatitudeAngle, oldHeightDistance);

        final BodyKinematics k48 = estimator.estimateAndReturnNew(timeInterval, c, oldC, speedN, speedE, speedD,
    ...
    public void testEstimateKinematics() throws InvalidSourceAndDestinationFrameTypeException,
        ...
        for (int t = 0; t < TIMES; t++) {
            ...
            final BodyKinematics k20 = new BodyKinematics();
            NEDKinematicsEstimator.estimateKinematics(timeInterval, frame, oldC, oldVn, oldVe, oldVd, oldPosition,
                    k20);
            ...
            final BodyKinematics k41 = new BodyKinematics();
            NEDKinematicsEstimator.estimateKinematics(TIME_INTERVAL_SECONDS, c, oldC, vn, ve, vd,
                    oldVn, oldVe, oldVd, latitudeAngle, height, oldLatitudeAngle, oldHeight, k41);

            final BodyKinematics k42 = new BodyKinematics();
            NEDKinematicsEstimator.estimateKinematics(timeInterval, c, oldC, vn, ve, vd,
                    oldVn, oldVe, oldVd, latitudeAngle, height, oldLatitudeAngle, oldHeight, k42);

            final BodyKinematics k43 = new BodyKinematics();
            NEDKinematicsEstimator.estimateKinematics(TIME_INTERVAL_SECONDS, c, oldC, vn, ve, vd,
                    oldVn, oldVe, oldVd, latitude, heightDistance, oldLatitude, oldHeightDistance, k43);

            final BodyKinematics k44 = new BodyKinematics();
            NEDKinematicsEstimator.estimateKinematics(timeInterval, c, oldC, vn, ve, vd,
                    oldVn, oldVe, oldVd, latitude, heightDistance, oldLatitude, oldHeightDistance, k44);
            ...
            final BodyKinematics k = estimateKinematics(TIME_INTERVAL_SECONDS, c, oldC, vn, ve, vd, oldVn, oldVe, oldVd,
                    latitude, height, oldLatitude, oldHeight);
    ...
    public void testEstimateKinematicsAndReturnNew() throws InvalidSourceAndDestinationFrameTypeException,
        ...
        for (int t = 0; t < TIMES; t++) {
            ...
            final BodyKinematics k1 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    oldC, vn, ve, vd, oldVn, oldVe, oldVd, latitude, height, oldLatitude, oldHeight);

            final Time timeInterval = new Time(TIME_INTERVAL_SECONDS, TimeUnit.SECOND);
            final BodyKinematics k2 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
                    vn, ve, vd, oldVn, oldVe, oldVd, latitude, height, oldLatitude, oldHeight);

            final NEDVelocity velocity = new NEDVelocity(vn, ve, vd);
            final NEDVelocity oldVelocity = new NEDVelocity(oldVn, oldVe, oldVd);
            final BodyKinematics k3 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS,
                    c, oldC, velocity, oldVelocity, latitude, height, oldLatitude, oldHeight);

            final BodyKinematics k4 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
            ...
            final Distance heightDistance = new Distance(height, DistanceUnit.METER);
            final Distance oldHeightDistance = new Distance(oldHeight, DistanceUnit.METER);
            final BodyKinematics k5 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    oldC, velocity, oldVelocity, latitudeAngle, heightDistance, oldLatitudeAngle, oldHeightDistance);

            final BodyKinematics k6 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
                    velocity, oldVelocity, latitudeAngle, heightDistance, oldLatitudeAngle, oldHeightDistance);

            final NEDPosition position = new NEDPosition(latitude, longitude, height);
            final NEDPosition oldPosition = new NEDPosition(oldLatitude, oldLongitude, oldHeight);
            final BodyKinematics k7 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    oldC, vn, ve, vd, oldVn, oldVe, oldVd, position, oldPosition);

            final BodyKinematics k8 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
            ...
            final Speed oldSpeedE = new Speed(oldVe, SpeedUnit.METERS_PER_SECOND);
            final Speed oldSpeedD = new Speed(oldVd, SpeedUnit.METERS_PER_SECOND);
            final BodyKinematics k9 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    oldC, speedN, speedE, speedD, oldSpeedN, oldSpeedE, oldSpeedD, position, oldPosition);

            final BodyKinematics k10 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
                    speedN, speedE, speedD, oldSpeedN, oldSpeedE, oldSpeedD, position, oldPosition);

            final BodyKinematics k11 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    oldC, velocity, oldVelocity, position, oldPosition);

            final BodyKinematics k12 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
                    velocity, oldVelocity, position, oldPosition);

            final BodyKinematics k13 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS,
                    frame, oldC, oldVn, oldVe, oldVd, oldLatitude, oldHeight);

            final BodyKinematics k14 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, frame, oldC,
                    oldVn, oldVe, oldVd, oldLatitude, oldHeight);

            final BodyKinematics k15 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS,
                    frame, oldC, oldVelocity, oldLatitude, oldHeight);

            final BodyKinematics k16 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, frame, oldC,
                    oldVelocity, oldLatitude, oldHeight);

            final BodyKinematics k17 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS,
                    frame, oldC, oldVelocity, oldLatitude, oldHeight);

            final BodyKinematics k18 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, frame, oldC,
                    oldVelocity, oldLatitude, oldHeight);

            final BodyKinematics k19 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS,
                    frame, oldC, oldVn, oldVe, oldVd, oldPosition);

            final BodyKinematics k20 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, frame, oldC,
                    oldVn, oldVe, oldVd, oldPosition);

            final BodyKinematics k21 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS,
                    frame, oldC, oldSpeedN, oldSpeedE, oldSpeedD, oldPosition);

            final BodyKinematics k22 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, frame, oldC,
                    oldSpeedN, oldSpeedE, oldSpeedD, oldPosition);

            final BodyKinematics k23 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS,
                    frame, oldC, oldVelocity, oldPosition);

            final BodyKinematics k24 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, frame, oldC,
                    oldVelocity, oldPosition);

            final BodyKinematics k25 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    vn, ve, vd, latitude, height, oldFrame);

            final BodyKinematics k26 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c,
                    vn, ve, vd, latitude, height, oldFrame);

            final BodyKinematics k27 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    velocity, latitude, height, oldFrame);

            final BodyKinematics k28 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, velocity,
                    latitude, height, oldFrame);

            final BodyKinematics k29 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    velocity, latitudeAngle, heightDistance, oldFrame);

            final BodyKinematics k30 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, velocity,
                    latitudeAngle, heightDistance, oldFrame);

            final BodyKinematics k31 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    vn, ve, vd, position, oldFrame);

            final BodyKinematics k32 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c,
                    vn, ve, vd, position, oldFrame);

            final BodyKinematics k33 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    speedN, speedE, speedD, position, oldFrame);

            final BodyKinematics k34 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c,
                    speedN, speedE, speedD, position, oldFrame);

            final BodyKinematics k35 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    velocity, position, oldFrame);

            final BodyKinematics k36 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, velocity,
                    position, oldFrame);

            final BodyKinematics k37 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS,
                    frame, oldFrame);

            final BodyKinematics k38 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, frame,
                    oldFrame);

            final BodyKinematics k39 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    oldC, speedN, speedE, speedD, oldSpeedN, oldSpeedE, oldSpeedD, latitude, height, oldLatitude,
                    oldHeight);

            final BodyKinematics k40 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
                    speedN, speedE, speedD, oldSpeedN, oldSpeedE, oldSpeedD, latitude, height, oldLatitude, oldHeight);

            final BodyKinematics k41 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    oldC, vn, ve, vd, oldVn, oldVe, oldVd, latitudeAngle, height, oldLatitudeAngle, oldHeight);

            final BodyKinematics k42 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
                    vn, ve, vd, oldVn, oldVe, oldVd, latitudeAngle, height, oldLatitudeAngle, oldHeight);

            final BodyKinematics k43 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    oldC, vn, ve, vd, oldVn, oldVe, oldVd, latitude, heightDistance, oldLatitude, oldHeightDistance);

            final BodyKinematics k44 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
                    vn, ve, vd, oldVn, oldVe, oldVd, latitude, heightDistance, oldLatitude, oldHeightDistance);

            final BodyKinematics k45 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c,
                    oldC, speedN, speedE, speedD, oldSpeedN, oldSpeedE, oldSpeedD, latitudeAngle, heightDistance,
                    oldLatitudeAngle, oldHeightDistance);

            final BodyKinematics k46 = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(timeInterval, c, oldC,
                    speedN, speedE, speedD, oldSpeedN, oldSpeedE, oldSpeedD, latitudeAngle, heightDistance,
                    oldLatitudeAngle, oldHeightDistance);
    ...
    public void testEstimateKinematicsWhenNegativeIntervalThrowsIllegalArgumentException()
        ...
        NEDKinematicsEstimator.estimateKinematicsAndReturnNew(-1.0, c, oldC, vn, ve, vd, oldVn, oldVe, oldVd,
                latitude, height, oldLatitude, oldHeight);
    }
    ...
    public void testEstimateKinematicsWhenInvalidCoordinateTransformationThrowsIllegalArgumentException()
        ...
        NEDKinematicsEstimator.estimateKinematicsAndReturnNew(TIME_INTERVAL_SECONDS, c, oldC, vn, ve, vd,
                oldVn, oldVe, oldVd, latitude, height, oldLatitude, oldHeight);
    }
    ...
    public void testEstimateKinematicsWhenZeroTimeIntervalReturnsZeroValues()
        ...
        final BodyKinematics k = NEDKinematicsEstimator.estimateKinematicsAndReturnNew(0.0, c, oldC,
                vn, ve, vd, oldVn, oldVe, oldVd, latitude, height, oldLatitude, oldHeight);
        ...
        final BodyKinematics k2 = estimateKinematics(0.0, c, oldC, vn, ve, vd, oldVn, oldVe, oldVd,
                latitude, height, oldLatitude, oldHeight);
